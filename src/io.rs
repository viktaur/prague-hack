...
pub fn angle_to_pulse_width(angle: f64) -> u16 {
    ...
    min + ((max - min) as f64 * angle / 180.0) as u64
}
    ...
    on_value: u16) -> Result<()>
{
    ...
    i2c.smbus_write_byte_data(0x00, 0x10)?; // Sleep mode
    i2c.smbus_write_byte_data(0xFE, prescale_val)?; // Prescale register
    i2c.smbus_write_byte_data(0x00, 0x20)?; // Wake up and auto-increment
    ...
    i2c.smbus_write_byte_data(on_l, (on_val & 0xFF) as u8)?;
    i2c.smbus_write_byte_data(on_l + 1, (on_val >> 8) as u8)?;
    i2c.smbus_write_byte_data(on_l + 2, (off_val & 0xFF) as u8)?;
    i2c.smbus_write_byte_data(on_l + 3, (off_val >> 8) as u8)?;
